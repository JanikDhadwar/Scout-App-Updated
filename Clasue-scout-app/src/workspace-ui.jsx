...
export function SyncPanel({ state, shellReady, onClose }) {
  ...
  return <section className="panel sync-panel" aria-label="Device and upload status">
    ...
    <div className="sync-summary"><div><strong>{state.pending}</strong><span>reports waiting to upload</span></div><div><strong>{state.connection==='online'?'Connected':'Offline'}</strong><span>{state.syncing?'Checking for saved reports…':'Uploads retry automatically with Scout open.'}</span></div></div>
    ...
    <p className="muted">Stay signed in on this browser. If Scout is closed, reopen it after reconnecting to finish uploads. Clearing browser data also clears unsynced reports.</p>
...
export function HomeDashboard({ team, user, role, mem, api, onNavigate, syncState, shellReady, onShowSync, onDeleteAccount, onDeleteTeam, onLeaveTeam }) {
  ...
  return <div className="dashboard page-enter">
    ...
    <div className="dashboard-grid">
      ...
      <section className="panel device-panel"><span className="eyebrow">Scout anywhere</span><h2>{syncState.pending?`${syncState.pending} report${syncState.pending===1?'':'s'} saved on this device`:shellReady&&syncState.readyAt?'Ready to go offline':'Prepare for offline scouting'}</h2><p>{syncState.pending?'Your reports will upload automatically when the server is reachable. Keep Scout open to finish syncing.':'Downloaded forms work without a connection. Your reports stay on this device until they reach the server.'}</p><div className="device-status"><span className={`status-dot ${syncState.connection==='offline'?'offline-dot':''}`}/>{syncState.connection==='online'?'Server connected':'Working from this device'}</div><button className="quiet-button" onClick={onShowSync}>Device & sync details →</button></section>
    </div>
